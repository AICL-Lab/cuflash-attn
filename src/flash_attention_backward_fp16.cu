...
__global__ void __launch_bounds__(128) compute_D_kernel_fp16(
    const half* __restrict__ dO,     // [batch*heads, seq_len, head_dim]
...
__global__ void __launch_bounds__(128) flash_attention_backward_dq_kernel_fp16(
    const half* __restrict__ Q,
    const half* __restrict__ K,
    const half* __restrict__ V,
    const half* __restrict__ L,
...
template __global__ void flash_attention_backward_dq_kernel_fp16<64, 64, 32>(
    const half*, const half*, const half*, const half*, const half*, const float*,
    half*, int, float, bool);
...
FlashAttentionError launch_flash_attention_backward_fp16(
    ...
    FlashAttentionError workspace_status = workspace.reserve(
        static_cast<size_t>(batch_heads) * static_cast<size_t>(seq_len));
    ...
    size_t dq_smem_size = (BLOCK_M * head_dim +
                           BLOCK_M * head_dim +
                           BLOCK_N * head_dim +
                           BLOCK_N * head_dim +
                           BLOCK_M * BLOCK_N +
    ...
    size_t dkdv_smem_size = (BLOCK_N * head_dim +
                             BLOCK_N * head_dim +
                             BLOCK_M * head_dim +
                             BLOCK_M * head_dim +
                             BLOCK_M * BLOCK_N +
                             ...
                             BLOCK_M +
                             BLOCK_M) * sizeof(float);
    size_t dq_smem_size_hd128 = (BLOCK_M_HD128 * head_dim +
    ...
    size_t dkdv_smem_size_hd128 = (BLOCK_N_HD128 * head_dim +
                                   BLOCK_N_HD128 * head_dim +
                                   BLOCK_M_HD128 * head_dim +
                                   BLOCK_M_HD128 * head_dim +
                                   BLOCK_M_HD128 * BLOCK_N_HD128 +
    ...
    if (head_dim == 32) {
        status = prepare_dynamic_smem_launch(
            reinterpret_cast<const void*>(flash_attention_backward_dq_kernel_fp16<BLOCK_M, BLOCK_N, 32>),
            dq_smem_size);
        if (status != FlashAttentionError::SUCCESS) {
            return status;
        }
        status = prepare_dynamic_smem_launch(
            reinterpret_cast<const void*>(flash_attention_backward_dkdv_kernel_fp16<BLOCK_M, BLOCK_N, 32>),
            dkdv_smem_size);
        if (status != FlashAttentionError::SUCCESS) {
            return status;
        }

        flash_attention_backward_dq_kernel_fp16<BLOCK_M, BLOCK_N, 32><<<dq_grid, block, dq_smem_size, stream>>>(
            Q, K, V, L, dO, D, dQ, seq_len, scale, causal);
        err = cudaGetLastError();
        if (err != cudaSuccess) {
            return FlashAttentionError::CUDA_ERROR;
        }

        flash_attention_backward_dkdv_kernel_fp16<BLOCK_M, BLOCK_N, 32><<<dkdv_grid, block, dkdv_smem_size, stream>>>(
            Q, K, V, L, dO, D, dK, dV, seq_len, scale, causal);
    } else if (head_dim == 64) {
        status = prepare_dynamic_smem_launch(
            reinterpret_cast<const void*>(flash_attention_backward_dq_kernel_fp16<BLOCK_M, BLOCK_N, 64>),
            dq_smem_size);
        if (status != FlashAttentionError::SUCCESS) {
            return status;
        }
        status = prepare_dynamic_smem_launch(
            reinterpret_cast<const void*>(flash_attention_backward_dkdv_kernel_fp16<BLOCK_M, BLOCK_N, 64>),
            dkdv_smem_size);
        if (status != FlashAttentionError::SUCCESS) {
            return status;
        }

        flash_attention_backward_dq_kernel_fp16<BLOCK_M, BLOCK_N, 64><<<dq_grid, block, dq_smem_size, stream>>>(
            Q, K, V, L, dO, D, dQ, seq_len, scale, causal);
        err = cudaGetLastError();
        if (err != cudaSuccess) {
            return FlashAttentionError::CUDA_ERROR;
        ...
        flash_attention_backward_dkdv_kernel_fp16<BLOCK_M, BLOCK_N, 64><<<dkdv_grid, block, dkdv_smem_size, stream>>>(
            Q, K, V, L, dO, D, dK, dV, seq_len, scale, causal);
    } else if (head_dim == 128) {
        status = prepare_dynamic_smem_launch(
            reinterpret_cast<const void*>(flash_attention_backward_dq_kernel_fp16<BLOCK_M_HD128, BLOCK_N_HD128, 128>),
            dq_smem_size_hd128);
        if (status != FlashAttentionError::SUCCESS) {
            return status;
        }
        status = prepare_dynamic_smem_launch(
            reinterpret_cast<const void*>(flash_attention_backward_dkdv_kernel_fp16<BLOCK_M_HD128, BLOCK_N_HD128, 128>),
            dkdv_smem_size_hd128);
        if (status != FlashAttentionError::SUCCESS) {
            return status;
        }

        flash_attention_backward_dq_kernel_fp16<BLOCK_M_HD128, BLOCK_N_HD128, 128><<<dq_grid_hd128, block, dq_smem_size_hd128, stream>>>(
            Q, K, V, L, dO, D, dQ, seq_len, scale, causal);
        err = cudaGetLastError();
        if (err != cudaSuccess) {
            return FlashAttentionError::CUDA_ERROR;
        ...
        flash_attention_backward_dkdv_kernel_fp16<BLOCK_M_HD128, BLOCK_N_HD128, 128><<<dkdv_grid_hd128, block, dkdv_smem_size_hd128, stream>>>(
            Q, K, V, L, dO, D, dK, dV, seq_len, scale, causal);
    }
...
FlashAttentionError flash_attention_backward_fp16(
    ...
    return launch_flash_attention_backward_fp16(Q, K, V, O, L, dO, dQ, dK, dV,
