...
__global__ void __launch_bounds__(128) flash_attention_backward_dq_kernel(
    ...
    store_tile_from_shared<BLOCK_M, HEAD_DIM>(dQ_tile, dQ_ptr, q_start, 0, seq_len, HEAD_DIM, HEAD_DIM);
}
...
template __global__ void flash_attention_backward_dq_kernel<64, 64, 32>(
    const float*, const float*, const float*, const float*, const float*, const float*,
    float*, int, float, bool);
template __global__ void flash_attention_backward_dq_kernel<64, 64, 64>(
    const float*, const float*, const float*, const float*, const float*, const float*,
    float*, int, float, bool);
template __global__ void flash_attention_backward_dq_kernel<64, 64, 128>(
    const float*, const float*, const float*, const float*, const float*, const float*,
    float*, int, float, bool);
template __global__ void flash_attention_backward_dq_kernel<16, 32, 128>(
    const float*, const float*, const float*, const float*, const float*, const float*,
    float*, int, float, bool);
...
FlashAttentionError launch_flash_attention_backward(
    ...
    size_t dq_smem_size_hd128 = (BLOCK_M_HD128 * head_dim +              // Q_tile
                                 ...
                                 BLOCK_M_HD128) * sizeof(float);         // D_tile
    ...
    if (head_dim == 32) {
        ...
        status = prepare_dynamic_smem_launch(
            reinterpret_cast<const void*>(flash_attention_backward_dkdv_kernel<BLOCK_M, BLOCK_N, 32>),
            dkdv_smem_size);
        if (status != FlashAttentionError::SUCCESS) {
            return status;
        }

        flash_attention_backward_dq_kernel<BLOCK_M, BLOCK_N, 32><<<dq_grid, block, dq_smem_size, stream>>>(
        ...
        flash_attention_backward_dkdv_kernel<BLOCK_M, BLOCK_N, 32><<<dkdv_grid, block, dkdv_smem_size, stream>>>(
        ...
        status = prepare_dynamic_smem_launch(
            reinterpret_cast<const void*>(flash_attention_backward_dkdv_kernel<BLOCK_M, BLOCK_N, 64>),
            dkdv_smem_size);
        if (status != FlashAttentionError::SUCCESS) {
            return status;
        }

        flash_attention_backward_dq_kernel<BLOCK_M, BLOCK_N, 64><<<dq_grid, block, dq_smem_size, stream>>>(
            Q, K, V, L, dO, D, dQ, seq_len, scale, causal);
        err = cudaGetLastError();
        if (err != cudaSuccess) {
            return FlashAttentionError::CUDA_ERROR;
        ...
        flash_attention_backward_dkdv_kernel<BLOCK_M, BLOCK_N, 64><<<dkdv_grid, block, dkdv_smem_size, stream>>>(
            Q, K, V, L, dO, D, dK, dV, seq_len, scale, causal);
    } else if (head_dim == 128) {
        status = prepare_dynamic_smem_launch(
            reinterpret_cast<const void*>(flash_attention_backward_dq_kernel<BLOCK_M_HD128, BLOCK_N_HD128, 128>),
            dq_smem_size_hd128);
        if (status != FlashAttentionError::SUCCESS) {
            return status;
        }
        status = prepare_dynamic_smem_launch(
            reinterpret_cast<const void*>(flash_attention_backward_dkdv_kernel<BLOCK_M_HD128, BLOCK_N_HD128, 128>),
            dkdv_smem_size_hd128);
        if (status != FlashAttentionError::SUCCESS) {
            return status;
        }

        flash_attention_backward_dq_kernel<BLOCK_M_HD128, BLOCK_N_HD128, 128><<<dq_grid_hd128, block, dq_smem_size_hd128, stream>>>(
            Q, K, V, L, dO, D, dQ, seq_len, scale, causal);
        err = cudaGetLastError();
        if (err != cudaSuccess) {
            return FlashAttentionError::CUDA_ERROR;
        ...
        flash_attention_backward_dkdv_kernel<BLOCK_M_HD128, BLOCK_N_HD128, 128><<<dkdv_grid_hd128, block, dkdv_smem_size_hd128, stream>>>(
            Q, K, V, L, dO, D, dK, dV, seq_len, scale, causal);
    }
