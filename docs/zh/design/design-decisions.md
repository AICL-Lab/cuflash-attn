...
| ID | 决策 | 主要驱动因素 | 次要权衡 |
|----|------|--------------|----------|
| ADR-1 | `head_dim ∈ {32, 64, 128}` | 编译期 tiling | 降低模型兼容性 |
| ADR-2 | FP16 → FP32 累加 | 数值稳定性 | 寄存器压力 |
| ADR-3 | `ctypes` 而非 `pybind11` | 零依赖构建 | 手动类型安全 |
| ADR-4 | OpenSpec 治理 | 正确性可追溯性 | 文档开销 |
| ADR-5 | `blockDim.x = 128` 固定 | Occupancy / warp 映射 | 小问题场景不灵活 |
| ADR-6 | O(N) SRAM tiling | 内存可扩展性 | Kernel 复杂度 |
