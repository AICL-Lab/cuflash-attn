...
TEST(DTypeTest, FP16BackwardBasic) {
    ...
    for (size_t i = 0; i < qkv_size; i++) {
        EXPECT_TRUE(std::isfinite(__half2float(h_dQ[i]))) << "dQ at index " << i << " is not finite";
        EXPECT_TRUE(std::isfinite(__half2float(h_dK[i]))) << "dK at index " << i << " is not finite";
        EXPECT_TRUE(std::isfinite(__half2float(h_dV[i]))) << "dV at index " << i << " is not finite";
    }
...
TEST(DTypeTest, FP16BackwardMatchesFP32) {
    ...
    err = flash_attention_backward(d_Q_f32, d_K_f32, d_V_f32, d_O_f32, d_L_f32,
        d_dO_f32, d_dQ_f32, d_dK_f32, d_dV_f32,
        batch_size, num_heads, seq_len, head_dim, scale, false, 0);
    ASSERT_EQ(err, FlashAttentionError::SUCCESS);
    cudaDeviceSynchronize();
    ...
    err = flash_attention_backward(d_Q_f16, d_K_f16, d_V_f16, d_O_f16, d_L_f16,
        d_dO_f16, d_dQ_f16, d_dK_f16, d_dV_f16,
        batch_size, num_heads, seq_len, head_dim, scale, false, 0);
    ASSERT_EQ(err, FlashAttentionError::SUCCESS);
    cudaDeviceSynchronize();
    ...
    cudaFree(d_Q_f32); cudaFree(d_K_f32); cudaFree(d_V_f32); cudaFree(d_O_f32); cudaFree(d_L_f32);
    cudaFree(d_dO_f32); cudaFree(d_dQ_f32); cudaFree(d_dK_f32); cudaFree(d_dV_f32);
    cudaFree(d_Q_f16); cudaFree(d_K_f16); cudaFree(d_V_f16); cudaFree(d_O_f16); cudaFree(d_L_f16);
    cudaFree(d_dO_f16); cudaFree(d_dQ_f16); cudaFree(d_dK_f16); cudaFree(d_dV_f16);
}
