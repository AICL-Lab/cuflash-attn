...
TEST(EdgeCaseTest, NullPointerInput) {
  auto err = cuflash::flash_attention_forward(nullptr, nullptr, nullptr, nullptr, nullptr, 1, 1, 64,
                                               64, 0.125f, false, 0);
...
TEST(EdgeCaseTest, ZeroSequenceLength) {
  ...
  auto err = cuflash::flash_attention_forward(d_Q, d_K, d_V, d_O, d_L, 1, 1, 0, 64, 0.125f, false,
                                               0);
...
TEST(EdgeCaseTest, ZeroBatchSize) {
  ...
  auto err = cuflash::flash_attention_forward(d_Q, d_K, d_V, d_O, d_L, 0, 1, 64, 64, 0.125f, false,
                                               0);
...
TEST(StressTest, LargeSequenceBackward) {
  ...
  auto devs = allocate_device_tensors<float>({qkv_size, qkv_size, qkv_size, qkv_size, l_size,
                                               qkv_size, qkv_size, qkv_size, qkv_size});
...
TEST(EdgeCaseTest, CausalMaskSeqLenOne) {
  ...
  auto err_causal = cuflash::flash_attention_forward(d_Q, d_K, d_V, d_O, d_L, batch_size, num_heads,
                                                      seq_len, head_dim, scale, true, stream);
