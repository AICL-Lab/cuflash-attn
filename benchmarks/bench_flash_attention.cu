...
static std::vector<T*> allocate_and_init(const std::vector<size_t>& sizes) {
  ...
  for (size_t size : sizes) {
    ...
    for (size_t i = 0; i < size; ++i) {
      h_data[i] = static_cast<T>(2.0f * static_cast<float>(rand()) /
...
static void FreeDeviceMemory(void* ptr) { cudaFree(ptr); }
...
static void BM_Forward_FP32(benchmark::State& state) {
  ...
  size_t l_size =
      static_cast<size_t>(batch_size) * num_heads * seq_len;
  ...
  for (auto _ : state) {
    auto err = cuflash::flash_attention_forward(d_Q, d_K, d_V, d_O, d_L,
                                                 batch_size, num_heads, seq_len,
                                                 head_dim, scale, false, stream);
...
static void BM_Backward_FP32(benchmark::State& state) {
  ...
  auto devs = allocate_and_init<float>(
      {qkv_size, qkv_size, qkv_size, qkv_size, l_size, qkv_size, qkv_size,
       qkv_size, qkv_size});
  ...
  for (auto _ : state) {
    auto err = cuflash::flash_attention_backward(
        d_Q, d_K, d_V, d_O, d_L, d_dO, d_dQ, d_dK, d_dV, batch_size, num_heads,
